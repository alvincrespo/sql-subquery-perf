...
BEGIN
    ...
    WHILE i < run_count DO
        SET start_time = (SELECT UNIX_TIMESTAMP(NOW(3)) * 1000); -- Start time in milliseconds
        ...
        SET end_time = (SELECT UNIX_TIMESTAMP(NOW(3)) * 1000); -- End time in milliseconds
        SET total_time = total_time + (end_time - start_time); -- Accumulate time
        SET i = i + 1;
    END WHILE;

    SELECT total_time / run_count AS average_execution_time_ms; -- Average execution time in milliseconds
END$$

-- Create stored procedure for inner join query
...
-- Create stored procedure for exists query
